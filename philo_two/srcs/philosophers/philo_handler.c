...
/*   Updated: 2020/09/09 16:45:25 by louis            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_philo		init_philo(t_args *args, int id)
{
	...
	philo.eat = sem_open("eat", O_CREAT, 0664, 1);
...
{
	sem_unlink("fork_message");
	if (!(args->fork_message = sem_open("fork_message", O_CREAT, 0664, 1)))
		return (EXIT_FAILURE);
	sem_unlink("picking");
